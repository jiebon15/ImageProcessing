...
int acumulator = 100;
int maxacumulator = 255;

Mat imgOriginal; Mat imgGray;

/// Function header
void bounding_rects(int, void* );
...
int main( int argc, char** argv )
{
	...
	createTrackbar("acumulator", "source", &acumulator, maxacumulator, bounding_rects );
  ...
  for( int i = 0; i< contours.size(); i++ )
     {
       ...
       rectangle( imgOriginal, boundRect[i].tl(), boundRect[i].br(), Scalar(0,0,255), 2, 8, 0 );
     }
